        ...
        <div class="container">
          <div class="page-inner">
            <div class="page-header">
              <h3 class="fw-bold mb-3">게시판 목록</h3>
            </div>
            <div class="row">
              <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="table-responsive">
         		    ...
         		    <% List<Notice> list = (List<Notice>) request.getAttribute("resultNoticeList");
           			 ...
           			 for (int i = 0; i < list.size(); i++) { %>
               			 <tr>
		                    <td>
		                        <%=list.get(i).getNotice_no()%>
		                    </td>
		                    <td>
		                        <%=list.get(i).getNotice_title()%>
		                    </td>
		                    <td>
		                        <%=list.get(i).getNotice_reg_date()%>
		                    </td>
		                    <td><input type="button" class="btn btn-success" value="수정"
		                            onclick="noticeAmendForm(<%= list.get(i).getNotice_no() %>);"></td>
		                    <td><input type="button" class="btn btn-danger" value="삭제"
		                            onclick="noticeDeleteForm(<%= list.get(i).getNotice_no() %>);"></td>
               			 </tr>
               				 <% } } else { %>
                    	 <tr>
                        <td colspan="0">공지사항이 없습니다.</td>
                   		 </tr>
                    <% } %>      
						</tbody>
                        
                                     <script>
                                     function noticeAmendForm(noticeNo){
                                        let url = '/notice/amend?notice_no=' + noticeNo;
                                          open(url, '_self');
                                     }
                                     function noticeDeleteForm(noticeNo){
                                        var result = confirm("정말로 삭제하시겠습니까?");
                                        if(result){
                                           let url = '/notice_admin/notice_deleteEnd?notice_no=' + noticeNo;
                                           open(url, '_self');
                                        }else{
                                           alert("삭제를 취소합니다.");
                                        }
