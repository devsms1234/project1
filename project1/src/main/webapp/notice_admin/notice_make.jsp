...
<html lang="en">
  ...
  <body>
        ...
        <div class="main-header">
          <div class="main-header-logo">
            ...
            <div class="page-header">
              <h3 class="fw-bold mb-3">게시물 등록</h3>
            </div>
            <div class="row">
              <div class="col-md-12">
                <div class="card">
                  <div class="card-header">
                    <div class="card-title">게시물 입력</div>
                  </div>
                  <div class="card-body">
                    <div class="row">
	...
	<form action="/notice_admin/notice_makeEnd" name="notice_add_form" method="post" enctype="multipart/form-data">
    <label for="notice_title">게시물 제목</label>
    <input type="text" class="form-control" id="notice_title" name="notice_title" placeholder="제목을 입력하세요."/>
    <br>

    <label for="notice_content">게시물 내용</label>
    <textarea class="form-control" id="notice_content" name="notice_content" rows="5"></textarea><br>
    <label for="notice_img">게시물 이미지</label>
    <input type="file" class="form-control-file" id="notice_img" name="notice_img" accept=".png,.jpg,.jpeg" /><br>

    <input type="button" class="btn btn-success" value="확인" onclick="notice_addBtn();">
    <input type="reset" class="btn btn-danger" value="비우기">
				</form>
				<script type="text/javascript">
				    function notice_addBtn(){
				        let form = document.notice_add_form;
				        if(!form.notice_title.value){
				            alert("게시물 제목을 입력하세요.");
				            form.notice_title.focus();
				        }else if(!form.notice_content.value) {
				            alert("게시물 내용을 입력 하세요");
				            form.notice_content.focus();
				        }else if(!form.notice_img.value){
				            alert("img 파일을 선택하세요.");
				            form.notice_img.focus();
				        }else {
				            const val = form.notice_img.value;
				            const idx = val.lastIndexOf('.');
				            const type = val.substring(idx+1).toLowerCase();
				            if(type == 'jpg' || type == 'jpeg' || type == 'png'){
				                form.submit();
				            }else{
				                alert("이미지 파일만 선택할 수 있습니다.");
				                form.notice_img.value = '';
				            }
				        }
				    }
				</script>
